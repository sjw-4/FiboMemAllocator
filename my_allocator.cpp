...
SegmentHeader* MyAllocator::splitSeg(SegmentHeader* curSeg, int requestedBlocks) {		//Takes the segment to be split and the number of blocks requested as inputs
...
bool MyAllocator::combineSegs(SegmentHeader* fSeg) {
	if (fSeg->length >= initSize)
		...
		SegmentHeader* buddySeg = (SegmentHeader*)((char*)fSeg - nToFibo(fiboToN(fSeg->length / basicBlockSize) - 1) * basicBlockSize);		//Assigns buddySeg where buddySeg would be if it exists
		if (buddySeg->CheckValid() && buddySeg->is_free && buddySeg->lr == 'l' && ((fiboToN(buddySeg->length / basicBlockSize) == fiboToN(fSeg->length / basicBlockSize) - 1) || fSeg->length == basicBlockSize)) {
			validateSeg(buddySeg);		//REMOVE BEFORE SUBMIT
			...
			validateSeg(fSeg);		//REMOVE BEFORE SUBMIT
		...
		if (buddySeg->CheckValid() && buddySeg->is_free && buddySeg->lr == 'r' && (fiboToN(buddySeg->length / basicBlockSize) == fiboToN(fSeg->length / basicBlockSize) + 1 || buddySeg->length == basicBlockSize)) {
			validateSeg(fSeg);		//REMOVE BEFORE SUBMIT
			if (!fls.at(fiboToN(fSeg->length / basicBlockSize) - 1)->Remove(fSeg))
...
int MyAllocator::getSegSize(SegmentHeader* addr) {
	SegmentHeader* seg = (SegmentHeader*)((char*)addr - sizeof(SegmentHeader));
	if (seg->CheckValid())
		return seg->length;
	else {
		cout << "Incorrect address requested" << endl;
		return -1;
	}
}

int MyAllocator::getTotSize() {
	return initSize;
}

void MyAllocator::checkFls() {		//Used for debug, remove before submit
	for(int i = 0; i < fls.size(); i++){
...
bool MyAllocator::validateSeg(SegmentHeader * checkSeg) {
	if (checkSeg < initP) {
		cout << "Segment of size " << checkSeg->length << " and isfree: " << checkSeg->is_free << " starts before original allocation." << endl;
		return false;
	}
	else if ((char*)checkSeg + checkSeg->length >(char*)initP + initSize) {
		cout << "Segment of size " << checkSeg->length << " and isfree: " << checkSeg->is_free << " extends beyond original allocation." << endl;
		return false;
	}
	else
		return true;
