...
class MyAllocator {
  ...
  */ 
  ...
  bool isFull();		//Remove before submit

  int getSegSize(SegmentHeader * addr);		//REMOVE BEFORE SUBMIT

  int getTotSize();		//REMOVE BEFORE SUBMIT

  void checkFls();		//REMOVE BEFORE SUBMIT

  bool validateSeg(SegmentHeader * checkSeg); //REMOVE BEFORE SUBMIT
};
